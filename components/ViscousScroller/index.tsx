...
import styles from './ViscousScroller.module.scss';
...
export const ViscousScroller = ({children, scrollConfig}: ViscousScrollerProps): ReactElement => {
  const contentRef = useRef<HTMLDivElement>(null);
  const scrollerRef = useRef<HTMLDivElement>(null);
  
  let currentStep : {index: number, point: ConfigPoint} = {index: 0, point: scrollConfig[0]};
  ...
  const updateCurrentStep = (scrollPosition: number) => {
    if (scrollPosition >= currentStep.point.toY && scrollConfig[currentStep.index + 1]) {
      currentStep = {index: currentStep.index + 1, point: scrollConfig[currentStep.index + 1]};
      updateCurrentFreezeHeight();
    }
    if (currentStep.index > 0 && scrollPosition < scrollConfig[currentStep.index - 1].toY) {
      currentStep = {index: currentStep.index - 1, point: scrollConfig[currentStep.index - 1]};
      updateCurrentFreezeHeight();
    }
  }
  ...
  const updateCurrentFreezeHeight = (): void => {
    ...
    for (let i = 0; i < currentStep.index; i++) {
      if (scrollConfig[i].mode === 'freeze') {
        _currentFreezeHeight += i === 0 ? scrollConfig[i].toY : scrollConfig[i].toY - scrollConfig[i - 1].toY;
      }
    }
  ...
  const handleScroll = (): void => {
    ...
    if (contentRef.current) {
      if (currentStep.point.mode === 'scroll') {
        contentRef.current.style.transform = `translateY(-${y - currentFreezeHeight}px)`;
      }
      updateCurrentStep(y);
    }
  }
